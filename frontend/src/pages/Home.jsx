...
const Home = () => {
  ...
  const handleCheckIn = async (passId) => {
    ...
    if (response.ok) {
      ...
      dispatch({ type: 'UPDATE_PASS', payload: { _id: passId, status: 'Checked In' } })
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Monitor active visitor passes, register new visitors, and schedule appointments.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="bg-slate-50 border border-slate-200/80 px-4 py-2 rounded-xl text-center min-w-27.5">
            <span className="block text-xs font-semibold text-slate-500 uppercase tracking-wider">Active Passes</span>
            <span className="text-xl font-bold text-blue-600">{passes ? passes.length : 0}</span>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            {passes && passes.length > 0 && (
              <span className="text-xs font-medium text-slate-500">
                Showing {passes.length} {passes.length === 1 ? 'pass' : 'passes'}
              </span>
            )}
          </div>
          ...
          {passes && passes.length === 0 && (
            <div className="bg-white rounded-2xl border border-dashed border-slate-300 p-12 text-center text-slate-500 shadow-xs">
              ...
              <p className="text-sm text-slate-500 mt-1 max-w-sm mx-auto">
                Register a visitor on the right to schedule their appointment and generate their digital pass.
              </p>
            </div>
          )}
              ...
              >
                <div>
                  <div className="flex items-center justify-between pb-3 mb-3 border-b border-slate-100">
                    ...
                    <span className="text-xs font-mono text-slate-400 truncate max-w-60" title={pass.appointmentId}>
                  ...
                  <div className="space-y-2 text-sm text-slate-600 mb-5 bg-slate-50/50 p-3 rounded-xl border border-slate-100">
                    <div className="flex justify-between items-center text-xs sm:text-sm">
                      ...
                      <span className="font-semibold text-slate-800 text-right">
                        {new Date(pass.validUntil).toLocaleString('en-GB', { dateStyle: 'short', timeStyle: 'short' })}
                      </span>
                    </div>
                ...
                <button onClick={() => handleOpenPDF(pass.pdfUrl)} className="w-full inline-flex items-center justify-center text-white bg-blue-600 hover:bg-blue-700 font-medium text-sm py-2.5 px-4 rounded-xl shadow-xs transition-colors text-center cursor-pointer"
                >
                  View Pass PDF
                </button>
