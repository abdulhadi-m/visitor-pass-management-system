                ...
                onResult={(result, error) => {
                  if (!!result && !isScanningRef.current) {
                    ...
                    try {
                      ...
                      scannedAppointmentId =
                        parsed.appointmentId || parsed._id || parsed.id;
                    } catch (e) {
                    ...
                    console.log(
                      "[QR Scanner] Extracted Appointment ID:",
                      scannedAppointmentId,
                    );

                    const matchedPass = passes?.find((pass) => {
                      const passAppId =
                        typeof pass.appointmentId === "object"
                          ? pass.appointmentId?._id
                          : pass.appointmentId;
                      return (
                        passAppId === scannedAppointmentId ||
                        pass._id === scannedAppointmentId
                      );
                    });

                    if (matchedPass) {
                      console.log("✅", matchedPass._id);
                      handleCheckIn(matchedPass._id);
                    } else {
                      console.warn(
                        "⚠️ [QR Scanner] No matching active pass found for:",
                        scannedAppointmentId,
                      );
                    ...
                    <div>
                      <div className="flex items-center justify-between pb-3 mb-3 border-b border-slate-100">
                        ...
                        >
                          Ref:{" "}
                          {appointmentRefId
                            ? String(appointmentRefId).slice(-6)
                            : "N/A"}
                        </span>
                      </div>
